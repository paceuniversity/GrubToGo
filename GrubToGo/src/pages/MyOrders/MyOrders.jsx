...
const MyOrders = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      // ⭐ Calculate total savings from past orders
      calculateSavings(past);
  ...
  // ⭐ Task 22.1 — Calculate Total Savings
  const calculateSavings = (pastOrders) => {
    let sum = 0;

    pastOrders.forEach(order => {
      // If your DB stores these fields:
      if (order.originalPrice && order.discountedPrice) {
        sum += (order.originalPrice - order.discountedPrice);
      }
      // If the order only has totalAmount, you may adjust logic here
    });
  ...
  return (
    <div className="my-orders-page">
      ...
      {/*  Savings Banner ONLY visible on “Past Orders” */}
      {activeTab === 'past' && (
        <div className="savings-banner">
          <p>Total Saved to Date: <strong>${totalSaved.toFixed(2)}</strong></p>
        </div>
      )}
